...
test("Slice 004 is one additive migration after Workstream foundation", () => {
  ...
  assert.equal(migrations.at(-7), "202608100001_create_workstream_activity_foundation.sql");
  assert.equal(migrations.at(-6), migrationName);
  assert.equal(migrations.at(-5), "202608100003_create_canonical_quote_scope_foundation.sql");
  assert.equal(migrations.at(-4), "202608100004_create_quote_composition_feedback.sql");
