...
const {
  ...
  TARGET_MIGRATIONS,
  VARIANT_FILENAME,
} = require("../production-convergence/004-r1/manifest");
const {
...
function usage() {
  return "Usage: node scripts/run-production-convergence-004-r1.js --describe|--preflight|--execute|--postflight";
}
...
async function executeConvergence({
  ...
  if (initialState === "ALREADY_APPLIED") {
    assertPostflightSnapshot(initial, expectedPrestate);
  ...
  try {
    ...
    assertPostflightSnapshot(poststate, expectedPrestate);
...
async function run({ argv = process.argv.slice(2), env = process.env, output = console.log } = {}) {
  ...
  try {
    ...
    output(JSON.stringify({
      ...
      ownerBackfillEligibility: snapshot.ownerBackfillEligibility,
    }));
    return 0;
  } finally {
    client.release();
