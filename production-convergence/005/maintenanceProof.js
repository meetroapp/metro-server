...
const MAINTENANCE_MECHANISM =
  "railway-immutable-maintenance-bridge-v1";

const PROOF_VERSION = 3;

const PROOF_FRESHNESS_MS =
  5 * 60 * 1000;
...
const ALLOWED_CONFIGURATION_DELTA =
  Object.freeze([
    "activeDeploymentId",
    "sourceImageDigest",
  ]);
...
function canonicalize(value) {
  ...
  throw blocked(
    "MAINTENANCE_PROOF_VALUE_INVALID"
  );
}
...
function validateMaintenanceProof(
  proof,
  {
    expectedProofSha256,
    ...
    expectedProductionRuntime =
      EXPECTED_PRODUCTION_RUNTIME,

    expectedProductionPrestate =
      PRODUCTION_PRESTATE,

    expectedOwnerEligibility =
      OWNER_BACKFILL_ELIGIBILITY,
  ...
  if (
    ...
    reasons.push(
      "EXPECTED_BRIDGE_DIGEST_INVALID"
    );
  }

  if (
    !STRICT_UUID_PATTERN.test(
      actualPreMaintenanceDeploymentId || ""
    )
  ) {
    reasons.push(
      "ACTUAL_PRE_MAINTENANCE_DEPLOYMENT_INVALID"
    );
  }
  ...
  if (
    ...
    reasons.push(
      "ACTUAL_PRE_MAINTENANCE_DEPLOYMENT_MISMATCH"
    );
  }
  ...
  if (
    ...
    reasons.push(
      "CURRENT_BRIDGE_DEPLOYMENT_INVALID"
    );
  }
  ...
  if (
    ...
    reasons.push(
      "CURRENT_BRIDGE_DIGEST_MISMATCH"
    );
  }
  ...
  if (
    ...
    reasons.push(
      "PRE_MAINTENANCE_AND_BRIDGE_DEPLOYMENT_REUSED"
    );
  }

  if (currentBridgeCurrent !== true) {
    reasons.push(
      "CURRENT_BRIDGE_NOT_CURRENT"
    );
  }
  ...
  const expected = {
    ...
    projectId:
      expectedProductionTarget.projectId,

    environmentId:
      expectedProductionTarget.environmentId,

    serviceId:
      expectedProductionTarget.backendServiceId,

    preMaintenanceDeploymentId:
      CERTIFIED_PRE_MAINTENANCE_DEPLOYMENT_ID,

    preMaintenanceServerSha:
      expectedProductionPrestate.serverSha,

    preMaintenanceImageDigest:
      expectedProductionPrestate.imageDigest,
    ...
    preMaintenanceRegion:
      expectedProductionRuntime.region,

    preMaintenanceReplicaCount:
      expectedProductionRuntime.replicaCount,

    preMaintenanceDomain:
      expectedProductionRuntime.domain,

    preMaintenancePort:
      expectedProductionRuntime.port,

    preMaintenanceDatabaseAttachment:
      expectedProductionRuntime.databaseAttachment,

    preMaintenanceHealthcheckPath:
      expectedProductionRuntime.healthcheckPath,
    ...
    ownerEligibilityFingerprint:
      expectedOwnerEligibility.eligibilityFingerprint,
  ...
  if (
    !Number.isInteger(
      proof.trafficProbeCount
    ) ||
    proof.trafficProbeCount < 8
  ) {
    reasons.push(
      "trafficProbeCount_INVALID"
    );
  }

  if (
    !HEX_SHA256_PATTERN.test(
      proof.configurationFingerprintBefore ||
        ""
    )
  ) {
    reasons.push(
      "configurationFingerprintBefore_INVALID"
    );
  }

  if (
    !HEX_SHA256_PATTERN.test(
      proof.configurationFingerprintMaintenance ||
        ""
    )
  ) {
    reasons.push(
      "configurationFingerprintMaintenance_INVALID"
    );
  }
  ...
  if (
    ...
    reasons.push(
      "CONFIGURATION_FINGERPRINT_DRIFT"
    );
  }

  if (
    JSON.stringify(
      proof.allowedConfigurationDelta
    ) !==
    JSON.stringify(
    ...
    reasons.push(
      "ALLOWED_CONFIGURATION_DELTA_INVALID"
    );
  }
  ...
  const bridgeCurrent =
    parseUtc(
      proof.bridgeBecameCurrentAtUtc,
      "bridgeBecameCurrentAtUtc",
      reasons
    );
  ...
  const oldInactive =
    parseUtc(
      proof.oldDeploymentInactiveAtUtc,
      "oldDeploymentInactiveAtUtc",
      reasons
    );
  ...
  const verified =
    parseUtc(
      proof.maintenanceVerifiedAtUtc,
      "maintenanceVerifiedAtUtc",
      reasons
    );
  ...
  const expires =
    parseUtc(
      proof.proofFreshnessExpiresAtUtc,
      "proofFreshnessExpiresAtUtc",
      reasons
    );
  ...
  if (
    ...
    if (
      !(
        preVerified <= bridgeCurrent &&
        bridgeCurrent <= oldInactive &&
        oldInactive <= verified
      )
    ) {
      reasons.push(
        "PROOF_TIME_ORDER_INVALID"
      );
    }

    if (
      verified >
      currentTime + CLOCK_SKEW_MS
    ) {
      reasons.push(
        "PROOF_FROM_FUTURE"
      );
  ...
  return Object.freeze({
    ...
    preMaintenanceDeploymentId:
      proof.preMaintenanceDeploymentId,

    bridgeDeploymentId:
      proof.bridgeDeploymentId,

    bridgeImageDigest:
      proof.bridgeImageDigest,

    databasePrestateFingerprint:
      proof.databasePrestateFingerprint,
...
function loadMaintenanceProof(
  env,
  { now = new Date() } = {}
) {
  const proofPath =
    String(
      env.MAINTENANCE_BRIDGE_PROOF_PATH ||
        ""
    );

  if (!path.isAbsolute(proofPath)) {
    throw blocked(
      "MAINTENANCE_PROOF_PATH_BLOCKED"
    );
  }
  ...
  try {
    stat =
      fs.lstatSync(proofPath);
  } catch {
    throw blocked(
      "MAINTENANCE_PROOF_FILE_MISSING"
    );
  }
  ...
  if (
    ...
    throw blocked(
      "MAINTENANCE_PROOF_FILE_INVALID"
    );
  }
  ...
  try {
    ...
    throw blocked(
      "MAINTENANCE_PROOF_JSON_INVALID"
    );
  }

  return validateMaintenanceProof(
    proof,
    {
      expectedProofSha256:
