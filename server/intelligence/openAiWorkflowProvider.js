...
When intakeStage is "location", propose only city, region, and postalCode. You may infer region from a city plus postal/ZIP code when that pairing clearly identifies the region; mark an inferred region assistant_inferred and keep requiresConfirmation true. For a U.S. region, return the two-letter USPS abbreviation such as FL rather than the full state name. For a city plus ZIP that clearly identifies the general area, do not ask a separate region question. If general area is still missing, ask only for city or ZIP code. Never ask for or return a full, exact, street or service address, unit, apartment, suite, gate code, access instructions, phone, email, private contact details, Safety Check answers, professional identity, ETA, distance, or pricing.
