...
async function bootstrapExistingCustomerRequestJob({
  client,
  request,
  relationship,
  logger = console,
  ...
  const homeownerUserId = Number(request?.user_id);
  const contractorProfileId = Number(
    request?.target_contractor_profile_id
  );
  const professionalUserId = Number(
    request?.target_professional_user_id
  );
  ...
  if (
    ...
    requestId <= 0 ||
    !Number.isSafeInteger(homeownerUserId) ||
    homeownerUserId <= 0 ||
    ...
    !Number.isSafeInteger(relationshipId) ||
    relationshipId <= 0 ||
    Number(relationship?.post_id) !== requestId ||
    Number(relationship?.homeowner_id) !==
      homeownerUserId ||
    Number(relationship?.contractor_id) !==
      contractorProfileId ||
    ...
    relationship?.ordinary_authority_source !==
      "existing_customer_request" ||
    String(
      relationship?.source_meetro_relationship_id || ""
    ).toLowerCase() !==
      String(
    ...
    throw new Error(
      "Existing Customer lifecycle Job source identity is invalid."
    );
  }
  ...
  if (!concernResult.rows[0]) {
    throw new Error(
      "Lifecycle-v2 Job creation requires preserved Reported Concern truth."
    );
  }
  ...
  const existing = await client.query(
    ...
    SELECT
      jobs.*,
      homeowner_participants.id
        AS homeowner_participant_id,
      professional_participants.id
        AS professional_participant_id
    FROM jobs
    LEFT JOIN relationship_participants
      homeowner_participants
      ON homeowner_participants.job_id = jobs.id
     AND homeowner_participants.request_relationship_id =
         jobs.source_request_relationship_id
     AND homeowner_participants.user_id = $3
     AND homeowner_participants.source_evidence_type =
         'existing_customer_request'
    LEFT JOIN relationship_participants
      professional_participants
      ON professional_participants.job_id = jobs.id
     AND professional_participants.request_relationship_id =
         jobs.source_request_relationship_id
     AND professional_participants.user_id = $4
     AND professional_participants.source_evidence_type =
         'existing_customer_request'
    WHERE jobs.source_type =
      'existing_customer_request'
      AND jobs.job_request_id = $1
      AND jobs.source_request_relationship_id = $2
    LIMIT 1
    FOR UPDATE OF jobs
    `,
    [
  ...
  if (existing.rows[0]) {
    ...
    if (
      Number(row.created_by_user_id) !==
        homeownerUserId ||
      !row.homeowner_participant_id ||
      !row.professional_participant_id
    ) {
      throw new Error(
        "Existing Customer lifecycle Job participant authority is incomplete."
      );
    }
    ...
    return {
      ...
      participants: [
        {
          id: row.homeowner_participant_id,
          user_id: homeownerUserId,
        },
        {
          id: row.professional_participant_id,
  ...
  const evidenceReference =
    `request:${requestId}:relationship:${relationshipId}`;

  const idempotencyPrefix =
    `existing-customer-request:${requestId}:relationship:${relationshipId}`;
  ...
  await client.query(
    ...
    [
      ...
      jobId,
      evidenceReference,
      `${idempotencyPrefix}:role:customer`,
      randomUUID(),
      professionalParticipantId,
      `${idempotencyPrefix}:role:professional`,
    ]
  );

  const customerCapabilityResult =
    await client.query(
      `
  ...
  const professionalCapabilityResult =
    await client.query(
      `
  ...
  const registeredCustomerCapabilities =
    customerCapabilityResult.rows.map(
      (row) => row.capability
    );

  const registeredProfessionalCapabilities =
    professionalCapabilityResult.rows.map(
      (row) => row.capability
    );

  const evaluationVisitCapabilityResult =
    await client.query(
      `
  ...
  const registeredEvaluationVisitCapabilities =
    new Set(
      evaluationVisitCapabilityResult.rows.map(
        (row) => row.capability
      )
    );
    ...
    for (const capability of capabilities) {
      await client.query(
        ...
        [
          ...
          capability,
          evidenceReference,
          `${idempotencyPrefix}:grant:${participantId}:${capability}`,
        ]
      );
    }
  }
  ...
  for (const [participantId, role, capabilities] of [
    ...
    for (const capability of capabilities) {
      if (
        !registeredEvaluationVisitCapabilities.has(
          capability
        )
      ) {
        continue;
      ...
      await client.query(
        ...
        [
          ...
          capability,
          evidenceReference,
          `${idempotencyPrefix}:evaluation-visit:${role}:${capability}`,
        ]
      );
    }
  }
  ...
  logger.info(
    "Existing Customer lifecycle Job created",
    {
      code:
        "EXISTING_CUSTOMER_REQUEST_JOB_CREATED",
      jobId,
      requestId,
      relationshipId,
      participantCount: 2,
      customerCapabilityCount:
        registeredCustomerCapabilities.length,
      professionalCapabilityCount:
        registeredProfessionalCapabilities.length,
      evaluationVisitCapabilityCount:
        registeredEvaluationVisitCapabilities.size,
    }
  );
